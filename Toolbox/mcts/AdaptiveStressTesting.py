...
class ASTParams:
    ...
    def __init__(self, max_steps, log_interval, n_itr=100):
...
class AdaptiveStressTest:
    ...
    Parameters
    ----------
    p : :py:class:`mcts.AdaptiveStressTesting.ASTParams`
        The AST parameters
    env : :py:class:`ast_toolbox.envs.go_explore_ast_env.GoExploreASTEnv`.
        The environment.
    top_paths : :py:class:`mcts.BoundedPriorityQueues`, optional
    ...
    def update(self, action):
        ...
        Parameters
        ----------
        action : :py:class:`mcts.AdaptiveStressTesting.ASTAction`
            The AST action.
    ...
    def logging(self):
        ...
        if self.step_count % self.params.log_interval == 0:
            ...
            for (topi, path) in enumerate(self.top_paths):
                top_paths[path[1]] = np.array([a.get() for a in path[0]])
    ...
    def explore_action(self, s, tree):
        ...
        Parameters
        ----------
        s : :py:class:`mcts.AdaptiveStressTesting.ASTState`
            The current state.
        tree : dict
            The searching tree.
    ...
    def transition_model(self):
        ...
        Returns
        ----------
        transition_model : :py:class:`mcts.MDP.TransitionModel`
...
class ASTState:
    ...
    parent : :py:class:`mcts.AdaptiveStressTesting.ASTState`
        The parent state.
    action : :py:class:`mcts.AdaptiveStressTesting.ASTAction`
...
def get_action_sequence(s):
    ...
    Returns
    ----------
    actions : list[:py:class:`mcts.AdaptiveStressTesting.ASTAction`]
